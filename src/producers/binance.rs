...
impl FromStr for Binance {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "Binance" | "binance" => Ok(Binance::new(std::path::PathBuf::from(
...
impl Binance {
    #[allow(dead_code)]
    /// Creates a `Binance` data producer.
    /// # Arguments
...
/// Returns the volume profile `Schema` for `Binance`
...
/// Returns the OHLC coloumn name as `String` for a `DataFrame` provided by `Binance`
/// # Arguments
...
/// Returns the OHLCV coloumn name as `String` for a `DataFrame` provided by `Binance`
/// # Arguments
/// * `c` - Column name we want to obtain a `String` value for
...
/// Returns the volume profile coloumn name as `String` for a `DataFrame` provided by `Binance`
/// # Arguments
/// * `c` - Column name we want to obtain a `String` value for
...
/// Returns the AggTrades coloumn name as `String` for a `DataFrame` provided by `Binance`
/// # Arguments
/// * `c` - Column name we want to obtain a `String` value for
...
mod tests {
    ...
    mod agg_trades {
        ...
        /// This unit test checks if the schema for the aggTrades data by binance matches. We also check the order of the schema elements.
        /// The `.csv` format is
        ///
        /// ```
        /// // idx = Index
        ...
        /// * Data Source: https://data.binance.vision/ (for more details: https://github.com/binance/binance-public-data/#where-do-i-access-it)
        ///
        /// # Note
        ...
        async fn test_schema() {
            ...
            let result = binance.schema(&LeafDir::AggTrades);
        ...
        /// This unit test checks if the enum types for the aggTrades data by binance matches with our id for the column names.
        /// The `.csv` format is
        ///
        /// ```
        /// // idx = Index
        ...
        async fn test_column_name_as_str() {
            ...
            assert_eq!(
                binance.column_name_as_str(&Columns::AggTrade(AggTradeColumnNames::AggTradeId)),
                String::from("atid")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::AggTrade(AggTradeColumnNames::Price)),
                String::from("px")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::AggTrade(AggTradeColumnNames::Quantity)),
                String::from("qx")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::AggTrade(AggTradeColumnNames::FirstTradeId)),
                String::from("ftid")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::AggTrade(AggTradeColumnNames::LastTradeId)),
                String::from("ltid")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::AggTrade(AggTradeColumnNames::Timestamp)),
                String::from("ts")
            );
            assert_eq!(
                binance
                    .column_name_as_str(&Columns::AggTrade(AggTradeColumnNames::BuyerEqualsMaker)),
                String::from("bm")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::AggTrade(
                    AggTradeColumnNames::BestTradePriceMatch
                )),
                String::from("btpm")
            );
        }

        /// This unit test checks if the order of the column ids actually match with the order of the loaded schema for the aggTrades data by binance.
        /// The `.csv` format is
        ///
        /// ```
        /// // idx = Index
        ...
        async fn test_column_name_as_int() {
            ...
            let result = binance.schema(&LeafDir::AggTrades);
            assert_eq!(
                binance.column_name_as_int(&Columns::AggTrade(AggTradeColumnNames::AggTradeId)),
                result.index_of("atid").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::AggTrade(AggTradeColumnNames::Price)),
                result.index_of("px").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::AggTrade(AggTradeColumnNames::Quantity)),
                result.index_of("qx").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::AggTrade(AggTradeColumnNames::FirstTradeId)),
                result.index_of("ftid").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::AggTrade(AggTradeColumnNames::LastTradeId)),
                result.index_of("ltid").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::AggTrade(AggTradeColumnNames::Timestamp)),
                result.index_of("ts").unwrap()
            );
            assert_eq!(
                binance
                    .column_name_as_int(&Columns::AggTrade(AggTradeColumnNames::BuyerEqualsMaker)),
                result.index_of("bm").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::AggTrade(
                    AggTradeColumnNames::BestTradePriceMatch
                )),
    ...
    mod ohlc {
        use super::*;
        use crate::config;
        /// This unit test checks if the schema for the OHLC data by binance matches. We also check the order of the schema elements.
        /// The `.csv` format is
        ///
        /// ```
        /// // idx = Index
        ...
        async fn test_schema() {
            ...
            let result = binance.schema(&LeafDir::Ohlc60m);
        ...
        /// This unit test checks if the enum types for the OHLC data by binance matches with our id for the column names.
        /// The `.csv` format is
        ///
        /// ```
        /// // idx = Index
        ...
        async fn test_column_name_as_str() {
            ...
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlc(OhlcColumnNames::OpenTime)),
                String::from("ots")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlc(OhlcColumnNames::Open)),
                String::from("open")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlc(OhlcColumnNames::High)),
                String::from("high")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlc(OhlcColumnNames::Low)),
                String::from("low")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlc(OhlcColumnNames::Close)),
                String::from("close")
            );
            // assert_eq!(
            ...
            // );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlc(OhlcColumnNames::CloseTime)),
                String::from("cts")
            );
            // assert_eq!(
        ...
        /// This unit test checks if the order of the column ids actually match with the order of the loaded schema for the OHLC data by binance.
        /// The `.csv` format is
        ///
        /// ```
        /// // idx = Index
        ...
        async fn test_column_name_as_int() {
            ...
            let result = binance.schema(&LeafDir::Ohlc60m);
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlc(OhlcColumnNames::OpenTime)),
                result.index_of("ots").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlc(OhlcColumnNames::Open)),
                result.index_of("open").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlc(OhlcColumnNames::High)),
                result.index_of("high").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlc(OhlcColumnNames::Low)),
                result.index_of("low").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlc(OhlcColumnNames::Close)),
            ...
            // );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlc(OhlcColumnNames::CloseTime)),
    ...
    mod ohlcv {
        ...
        async fn test_schema() {
            ...
            let result = binance.schema(&LeafDir::Ohlcv60m);
            assert_eq!(result.index_of("ots").unwrap(), 0);
            assert_eq!(result.index_of("open").unwrap(), 1);
            assert_eq!(result.index_of("high").unwrap(), 2);
            assert_eq!(result.index_of("low").unwrap(), 3);
        ...
        async fn test_column_name_as_str() {
            ...
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlcv(OhlcvColumnNames::OpenTime)),
                String::from("ots")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlcv(OhlcvColumnNames::Open)),
                String::from("open")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlcv(OhlcvColumnNames::High)),
                String::from("high")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlcv(OhlcvColumnNames::Low)),
                String::from("low")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlcv(OhlcvColumnNames::Close)),
                String::from("close")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlcv(OhlcvColumnNames::Volume)),
                String::from("vol")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Ohlcv(OhlcvColumnNames::CloseTime)),
                String::from("cts")
            );
            // assert_eq!(
        ...
        /// This unit test checks if the order of the column ids actually match with the order of the loaded schema for the Ohlcv data by binance.
        /// The `.csv` format is
        ///
        /// ```
        /// // idx = Index
        ...
        async fn test_column_name_as_int() {
            ...
            let result = binance.schema(&LeafDir::Ohlcv60m);
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlcv(OhlcvColumnNames::OpenTime)),
                result.index_of("ots").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlcv(OhlcvColumnNames::Open)),
                result.index_of("open").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlcv(OhlcvColumnNames::High)),
                result.index_of("high").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlcv(OhlcvColumnNames::Low)),
                result.index_of("low").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlcv(OhlcvColumnNames::Close)),
                result.index_of("close").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlcv(OhlcvColumnNames::Volume)),
                result.index_of("vol").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Ohlcv(OhlcvColumnNames::CloseTime)),
                result.index_of("cts").unwrap()
            );
            // assert_eq!(
    ...
    mod vol {
        ...
        async fn test_column_name_as_str() {
            ...
            assert_eq!(
                binance.column_name_as_str(&Columns::Vol(VolumeProfileColumnNames::Price)),
                String::from("px")
            );
            assert_eq!(
                binance.column_name_as_str(&Columns::Vol(VolumeProfileColumnNames::Quantity)),
                String::from("qx")
            );
        }
        ...
        async fn test_column_name_as_int() {
            ...
            let result = binance.schema(&LeafDir::Vol);
            assert_eq!(
                binance.column_name_as_int(&Columns::Vol(VolumeProfileColumnNames::Price)),
                result.index_of("px").unwrap()
            );
            assert_eq!(
                binance.column_name_as_int(&Columns::Vol(VolumeProfileColumnNames::Quantity)),
                result.index_of("qx").unwrap()
            );
        }
    }
