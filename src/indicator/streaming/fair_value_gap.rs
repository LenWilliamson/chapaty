...
/// Defines the time to live (ttl) condition under which a Fair Value Gap
/// expires.
...
pub enum GapInteraction {
    ...
    Miss,
    /// The candle's price range entered the gap, but did not pierce the far
    /// boundary.
    Touch,
    /// The candle's price range completely pierced the required boundary to
    /// fill the gap.
    Fill,
}
...
impl<S: FairValueGapState> FairValueGap<S> {
    ...
    /// The three candles that formed this gap, chronological `[first,
    /// displacement, last]`.
    ...
    /// The middle candle. The impulse/displacement bar whose move opened the
    /// gap.
    ...
    ///
    /// This is the breakout extreme under the whole movement reading of a
    /// bullish setup, where the entire three candle leg is considered
    /// rather than the displacement bar alone. It is always greater than or
    /// equal to [`Self::displacement_high`].
    ...
    ///
    /// This is the breakout extreme under the whole movement reading of a
    /// bearish setup. It is always less than or equal to
    /// [`Self::displacement_low`].
...
impl<S: FairValueGapState> FairValueGap<S> {
    fn map<NewState: FairValueGapState, F>(self, f: F) -> FairValueGap<NewState>
    where
        F: FnOnce(S) -> NewState,
    {
        FairValueGap {
    ...
    /// Evaluates how a given candle's price action interacts with the gap's
    /// price zone.
    ///
    /// # The Overlap Logic (Filtering Breakaway Gaps)
    /// A candle's traded range is a continuous interval defined as `[low,
    /// high]`. The gap's price zone is defined as `[bottom, top]`.
    ///
    /// For a candle to interact with the gap, the market must have physically
    /// traded inside that zone. Mathematically, two 1-dimensional intervals
    /// `[A, B]` and `[C, D]` intersect if and only if `A < D` AND `B > C`.
    ///
    /// Applying this to our market data:
    /// `candle.low < gap.top` AND `candle.high > gap.bottom`
    ///
    /// **Why this is critical:**
    /// In markets that close (like traditional equities) or over weekends (like
    /// Forex), the price can open drastically lower or higher than the
    /// previous close. If a Bullish Gap exists at `[10.0, 15.0]`, and the
    /// market violently crashes overnight to open at `5.0` and wicks to a
    /// high of `8.0`, the price is technically "below" the gap. But because
    /// `candle.high (8.0)` is NOT `> gap.bottom (10.0)`, the overlap check
    /// correctly identifies that the market teleported _over_ the
    /// zone without ever actually trading inside it. It remains an untouched
    /// Miss.
    fn evaluate_interaction(&self, candle: &Ohlcv, price_source: PriceSource) -> GapInteraction {
        ...
        let is_filled = match (self.direction, price_source) {
            (FairValueGapDirection::Bullish, PriceSource::HighLow) => candle.low <= self.bottom,
            (FairValueGapDirection::Bearish, PriceSource::HighLow) => candle.high >= self.top,
            (FairValueGapDirection::Bullish, PriceSource::OpenClose) => {
                candle.open.min(candle.close) <= self.bottom
            }
            (FairValueGapDirection::Bearish, PriceSource::OpenClose) => {
                candle.open.max(candle.close) >= self.top
            }
        };
...
impl FairValueGap<OpenState> {
    /// Evaluates the incoming indexed candle against the open gap, considering
    /// TTL.
    fn process_candle(
        self,
        ...
        let updated_gap = match self.evaluate_interaction(candle, price_source) {
            GapInteraction::Fill => {
                // Early return: If it fully fills, it closes immediately before
                // TTL checks.
...
impl Default for StreamingFairValueGap {
    fn default() -> Self {
        Self {
            ...
            buffer: VecDeque::with_capacity(PATTERN_LENGTH + 1),
            active_gaps: Vec::new(),
            closed_gaps: Vec::new(),
...
impl StreamingIndicator for StreamingFairValueGap {
    ...
    fn update(&mut self, indexed_candle: Self::Input) -> Self::Output<'_> {
        ...
        let price_source = self.price_source;

        self.active_gaps.retain_mut(|gap_ref| {
            match gap_ref.process_candle(&indexed_candle, ttl, price_source) {
                FairValueGapStatus::Open(updated_gap) => {
                    *gap_ref = updated_gap;
                    true // Keep in active
                }
...
mod tests {
    ...
    fn simultaneous_bullish_and_bearish_gap_is_impossible() {
        ...
        //
        // As C1.low is greater than C3.high (gap_down) and C3.high >= C3.low
        // (valid candle), we get C1.low > C3.high >= C3.low > C1.high >
        // 0, by extending the left side of the inequality of gap_up.
        //
        // This transitively means C1.low > C1.high, which is a contradiction.
        // Hence, the gap_up and gap_down cannot both exist
        // simultaneously.

        let mut fvg = StreamingFairValueGap::default().with_min_gap_size(0.1);

        // Feed an erratic sequence to ensure the math holds and the
    ...
    fn partial_fill_updates_active_state_and_clamps() {
        ...
        // 3. Lesser Fill: Wick down to 14.0 (20% fill). Should NOT reduce
        //    max_fill.
    ...
    fn boundary_exact_tick_is_a_miss() {
        ...
        // Because process_candle uses `candle.low < self.top`, this evaluates
        // to false. It is mathematically defined as a Miss, NOT a
        // touch/partial fill.
    ...
    fn multiple_gaps_tracked_and_filled_independently() {
        ...
        // 3. Price drops to 20. This completely fills Gap B (25->30), but only
        //    misses Gap A (10->15)
        ...
        // Verify Gap A is still active and untouched (passed by value since it
        // is Copy)
    ...
    fn expired_state_preserves_partial_fill_history() {
        ...
        // 3. Expiration: Next bar runs away but triggers the 2-bar expiration
        //    limit.
        indicator.update(candle(5, "2026-05-26T10:05:00Z", 20., 25., 20., 22.));
        ...
        // Verify that the ExpiredState successfully inherited the fill data
        // from OpenState
        let expired = indicator.expired_gaps()[0];
        assert_eq!(
    ...
    fn ttl_policy_filled_never_expires() {
        // TtlPolicy::Filled ist der Standard. Die Lücke darf niemals von allein
        // verfallen.
        let mut indicator = StreamingFairValueGap::default()
            .with_min_gap_size(1.0)
        ...
        // 2. Einen gewaltigen Sprung in die Zukunft simulieren (Index 1000, 10
        //    Stunden später)
    ...
    fn simultaneous_full_fill_and_expiration_results_in_closed_gap() {
        // If a gap completely fills on the exact same candle that triggers its
        // expiration, the fill wins. The price action happened *during* the
        // candle.
        let mut indicator = StreamingFairValueGap::default()
            .with_min_gap_size(1.0)
            .with_ttl_policy(TtlPolicy::Bars(2));
        ...
        // AT THE EXACT SAME TIME, it has a violent wick down to 5.0, fully
        // covering the gap.
    ...
    fn simultaneous_partial_fill_and_expiration_preserves_final_action() {
        // If a gap partially fills on the exact same candle that triggers its
        // expiration, it must expire, BUT it must successfully capture the
        // partial fill from its final moments.
        let mut indicator = StreamingFairValueGap::default()
            .with_min_gap_size(1.0)
            .with_ttl_policy(TtlPolicy::Bars(2));
        ...
        // 3. The expiry candle! Index 5 triggers the 2-bar expiration.
        // It drops to 12.5, filling exactly 50% of the gap right before time
        // runs out.
    ...
    fn breakaway_gaps_do_not_touch_or_fill_fvg() {
        ...
        // If the market completely teleports over the FVG zone without trading
        // inside it, the gap must remain open and untouched.
        let mut indicator = StreamingFairValueGap::default().with_min_gap_size(1.0);
    ...
    #[test]
    fn gap_interaction_evaluates_high_low_overlap_and_fills_correctly() {
        // === 1. Bullish Gap Setup (Top=15.0, Bottom=10.0) ===
        // Window indices must be the contiguous triple ending at
        // creation_index. creation_index = 2 => window = [0, 1, 2]. The
        // candles form a real bullish gap: rhs.low (15) > lhs.high
        // (10), with bottom = lhs.high, top = rhs.low.
        let bullish_gap = FairValueGap {
            ...
            bottom: Price(10.0),
            window: [
                candle(0, "2026-05-24T10:00:00Z", 8., 10., 5., 9.).candle, /* lhs: high = 10
                                                                            * (gap bottom) */
                candle(1, "2026-05-24T10:01:00Z", 9., 14., 9., 13.).candle, /* mid: displacement
                                                                             * impulse */
                candle(2, "2026-05-24T10:02:00Z", 15., 18., 15., 16.).candle, /* rhs: low = 15
                                                                               * (gap top) */
            ],
            state: OpenState::default(),
        };
        ...
        let interaction = bullish_gap.evaluate_interaction(&miss_above, PriceSource::HighLow);
        ...
        let interaction = bullish_gap.evaluate_interaction(&breakaway_below, PriceSource::HighLow);
        assert_eq!(interaction, GapInteraction::Miss);

        // C. Bullish Touch (Wick enters the gap: low is 12.0)
        let touch_candle = candle(5, "2026-05-24T10:05:00Z", 18., 18., 12., 15.).candle;
        let interaction = bullish_gap.evaluate_interaction(&touch_candle, PriceSource::HighLow);
        ...
        let interaction = bullish_gap.evaluate_interaction(&fill_candle, PriceSource::HighLow);
        ...
        let bearish_gap = FairValueGap {
            ...
            bottom: Price(15.0),
            window: [
                candle(0, "2026-05-24T10:00:00Z", 22., 25., 20., 21.).candle, /* lhs: low = 20
                                                                               * (gap top) */
                candle(1, "2026-05-24T10:01:00Z", 21., 22., 16., 17.).candle, /* mid: displacement impulse */
                candle(2, "2026-05-24T10:02:00Z", 14., 15., 10., 11.).candle, /* rhs: high = 15
                                                                               * (gap bottom) */
            ],
            state: OpenState::default(),
        };
        ...
        assert_eq!(
            bearish_gap.evaluate_interaction(&miss_below, PriceSource::HighLow),
            GapInteraction::Miss
        );
        ...
        assert_eq!(
            bearish_gap.evaluate_interaction(&breakaway_above, PriceSource::HighLow),
            GapInteraction::Miss
        );
        ...
        assert_eq!(
            bearish_gap.evaluate_interaction(&touch_bear, PriceSource::HighLow),
            GapInteraction::Touch
        );

        // D. Bearish Fill (Wick spikes above the top of 20.0)
        let fill_bear = candle(6, "2026-05-24T10:06:00Z", 10., 21., 10., 12.).candle;
        let interaction = bearish_gap.evaluate_interaction(&fill_bear, PriceSource::HighLow);
        assert_eq!(interaction, GapInteraction::Fill);
        assert!(interaction.is_touch());
        assert!(interaction.is_fill());
    }
    ...
    #[test]
    fn gap_interaction_evaluates_open_close_fills_correctly() {
        // === 1. Bullish Gap Setup (Top=15.0, Bottom=10.0) ===
        let bullish_gap = FairValueGap {
            direction: FairValueGapDirection::Bullish,
            creation_time: ts("2026-05-24T10:02:00Z"),
            ...
            bottom: Price(10.0),
            window: [
                candle(0, "2026-05-24T10:00:00Z", 8., 10., 5., 9.).candle,
                candle(1, "2026-05-24T10:01:00Z", 9., 14., 9., 13.).candle,
                candle(2, "2026-05-24T10:02:00Z", 15., 18., 15., 16.).candle,
            ],
            state: OpenState::default(),
        };

        // A. Wick Fill, but Body Touch (Price wicks below 10.0, but body closes
        // inside at 11.0)
        let wick_fill_body_touch = candle(3, "2026-05-24T10:03:00Z", 14., 14., 8., 11.).candle;

        assert_eq!(
            bullish_gap.evaluate_interaction(&wick_fill_body_touch, PriceSource::OpenClose),
            GapInteraction::Touch,
            "Wick filled, but body stayed above bottom; under OpenClose this is a Touch"
        );
        assert_eq!(
            bullish_gap.evaluate_interaction(&wick_fill_body_touch, PriceSource::HighLow),
            GapInteraction::Fill,
            "Same candle: wick fills under HighLow even though body only touches"
        );

        // B. Body Fill via Bearish Candle (Opens inside gap at 12.0, closes
        // below at 9.0)
        let body_fill_bearish = candle(4, "2026-05-24T10:04:00Z", 12., 13., 8., 9.).candle;
        assert_eq!(
            bullish_gap.evaluate_interaction(&body_fill_bearish, PriceSource::OpenClose),
            GapInteraction::Fill,
            "Bearish body closed below bottom; should be Fill"
        );

        // C. Body Fill via Bullish Candle (Opens below gap at 9.0, closes
        // inside at 12.0)
        let body_fill_bullish = candle(5, "2026-05-24T10:05:00Z", 9., 13., 8., 12.).candle;
        assert_eq!(
            bullish_gap.evaluate_interaction(&body_fill_bullish, PriceSource::OpenClose),
            GapInteraction::Fill,
            "Bullish body opened below bottom; should be Fill"
        );
        ...
        let bearish_gap = FairValueGap {
            ...
            bottom: Price(15.0),
            window: [
                candle(0, "2026-05-24T10:00:00Z", 22., 25., 20., 21.).candle,
                candle(1, "2026-05-24T10:01:00Z", 21., 22., 16., 17.).candle,
                candle(2, "2026-05-24T10:02:00Z", 14., 15., 10., 11.).candle,
            ],
            state: OpenState::default(),
        };

        // A. Wick Fill, but Body Touch (Price wicks above 20.0, but body closes
        // inside at 19.0)
        let wick_fill_body_touch_bear =
            candle(3, "2026-05-24T10:03:00Z", 16., 22., 15., 19.).candle;

        assert_eq!(
            bearish_gap.evaluate_interaction(&wick_fill_body_touch_bear, PriceSource::OpenClose),
            GapInteraction::Touch,
            "Wick filled, but body stayed below top; under OpenClose this is a Touch"
        );
        assert_eq!(
            bearish_gap.evaluate_interaction(&wick_fill_body_touch_bear, PriceSource::HighLow),
            GapInteraction::Fill,
            "Same candle: wick fills under HighLow even though body only touches"
        );

        // B. Body Fill via Bullish Candle (Opens inside gap at 18.0, closes
        // above at 21.0)
        let body_fill_bullish_bear = candle(4, "2026-05-24T10:04:00Z", 18., 22., 17., 21.).candle;
        assert_eq!(
            bearish_gap.evaluate_interaction(&body_fill_bullish_bear, PriceSource::OpenClose),
            GapInteraction::Fill,
            "Bullish body closed above top; should be Fill"
        );

        // C. Body Fill via Bearish Candle (Opens above gap at 21.0, closes
        // inside at 18.0)
        let body_fill_bearish_bear = candle(5, "2026-05-24T10:05:00Z", 21., 22., 17., 18.).candle;
        assert_eq!(
            bearish_gap.evaluate_interaction(&body_fill_bearish_bear, PriceSource::OpenClose),
            GapInteraction::Fill,
            "Bearish body opened above top; should be Fill"
        );
    }
    ...
    fn window_survives_state_transitions() {
        ...
        // Full fill (wick below bottom 10.0) -> migrates to closed; map() must
        // carry the window.
    ...
    fn window_displacement_equals_movement_when_mid_is_extreme() {
        ...
        // fold returns the displacement bar's own value rather than perturbing
        // it.
        let mut indicator = StreamingFairValueGap::default().with_min_gap_size(1.0);

        // mid carries both the window high (25) and the window low (9). lhs and
        // rhs stay strictly inside that range, so neither can win the max or
        // min.
    ...
    fn window_movement_open_and_close_span_the_leg() {
        // movement_open is the open of the first candle and movement_close is
        // the close of the last candle, together bounding the leg from
        // its start to its end. This is the only test that reads these
        // two accessors, so it also guards their wiring to
        // first()/last() through state transitions.
        let mut indicator = StreamingFairValueGap::default().with_min_gap_size(1.0);
        ...
        // And they must survive migration to closed, like the rest of the
        // window.
