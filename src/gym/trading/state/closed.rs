...
impl Trade<Closed> {
    /// Calculates the duration the trade was active (in milliseconds/seconds
    /// depending on TS basis).
    #[must_use]
    ...
    /// Returns `0.0` if the cost basis is zero (should not happen in valid
    /// trades).
    #[must_use]
    pub fn roi(&self) -> f64 {
