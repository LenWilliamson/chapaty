...
use crate::{
    common::wrappers::{unwrap_float64, unwrap_uint32},
    enums::columns::ProfitAndLossColumnNames,
};
...
fn get_number_of_trades_from_summary(df: DataFrame, trade: &str) -> u32 {
    ...
    unwrap_uint32(&trades["counts"].get(0).unwrap())
}
...
pub fn total_number_trades(df: DataFrame) -> u32 {
    ...
    let total = counts
        ...
        .collect()
        .unwrap();
    unwrap_uint32(&total["counts"].get(0).unwrap())
}
...
pub fn avg_win(df: DataFrame) -> f64 {
    ...
    unwrap_float64(&res[pl_dollar_col.as_str()].get(0).unwrap())
}
...
pub fn accumulated_profit(df: DataFrame, initial: f64) -> Vec<f64> {
    let pl_dollar_col = ProfitAndLossColumnNames::PlDollar.to_string();
    df[pl_dollar_col.as_str()]
        .iter()
        .fold(vec![initial], |mut acc, val| {
            acc.push(unwrap_float64(&val) + acc.last().unwrap());
            acc
        })
}
